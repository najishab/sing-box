...
// NOTE: this is a version ported from sing-box-extended's common/tls/openvpn_client.go,
// adapted for this (older) sing-box base which does not yet have KTLSClientConfig.
// kernel_tx / kernel_rx (kTLS offload) are intentionally unsupported here; they are
// not relevant on Android anyway. If options.KernelTx/KernelRx are set, we return an error
...
func NewOpenVPNClient(ctx context.Context, logger logger.ContextLogger, options option.OpenVPNTLSOptions) (Config, error) {
	...
	if options.KernelTx || options.KernelRx {
		return nil, E.New("openvpn: kernel_tx/kernel_rx (kTLS) is not supported in this build")
	}
